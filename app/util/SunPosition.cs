...
public static class SunPosition {
     ...
     * found here:
     * http://www.astro.uio.no/~bgranslo/aares/calculate1.html
     ...
     */
    public static double[] CalcSunPosition(
        ...
        double siderealTimeHours =
            6.6974 + 2400.0513 * julianCenturies;

        double siderealTimeUT = siderealTimeHours +
            (366.2422 / 365.2422) *
            (double)dateTime.TimeOfDay.TotalHours;
        ...
        // Solar Coordinates
        double meanLongitude = CorrectAngle((280.466 +
            36000.77 * julianCenturies) * Deg2Rad);

        double meanAnomaly = CorrectAngle((357.529 +
            35999.05 * julianCenturies) * Deg2Rad);

        double equationOfCenter = ((1.915 -
            0.005 * julianCenturies) * Math.Sin(meanAnomaly) +
        ...
        double obliquity =
            (23.439 - 0.013 * julianCenturies) * Deg2Rad;
        ...
        return new double[] { altitude * Rad2Deg, azimuth * Rad2Deg };
        // Altitude
    ...
    */
    private static double CorrectAngle(double angleInRadians) {
        if (angleInRadians < 0)
            return 2 * Math.PI -
                (Math.Abs(angleInRadians) % (2 * Math.PI));
